...
class MazeEditor:
    ...
    def _setup_gui(self):
        ...
        Button(top_control_frame, text="Save Maze", command=self.save_maze_text).pack(side=tk.LEFT, padx=5)
        Button(top_control_frame, text="Load Maze", command=self.load_maze_text).pack(side=tk.LEFT, padx=5)
        Button(top_control_frame, text="Load from GitHub", command=self.fetch_github_maze_list).pack(side=tk.LEFT, padx=5)
        # Right controls
    ...
    def _create_color_key(self):
        ...
        self.key_frame.columnconfigure(2, weight=2); self.key_frame.columnconfigure(3, weight=1)
        self.key_frame.columnconfigure(4, weight=1)
        font_size = 8
    ...
    def handle_resize(self, width, height):
        """Recalculates cell size and redraws."""
        ...
        if new_cell_size != self.cell_visual_size_px:
            ...
            self.find_and_draw_routes() # Redraw required
    ...
    def clear_routes(self):
        ...
        self.canvas.delete("route_left", "route_shortest", "route_straightest",
                           "route_diagonal", "route_smoothed")

    # --- Coordinate Conversion ---
    def cell_to_pixel(self, r, c):
        x = MARGIN + c * self.cell_visual_size_px
        y = MARGIN + r * self.cell_visual_size_px
        return x, y
    ...
    def wall_midpoint_to_pixel(self, r_cell, c_cell, direction4):
        cell_size = self.cell_visual_size_px; center_x, center_y = self.cell_center_to_pixel(r_cell, c_cell)
    ...
    def get_wall_from_coords(self, click_x, click_y):
        ...
        r_approx_cell = int((click_y - MARGIN) / cell_size)
        c_approx_cell = int((click_x - MARGIN) / cell_size)
        for r_wall_check in range(max(0, r_approx_cell), min(GRID_SIZE + 1, r_approx_cell + 2)):
            for c_cell_check in range(max(0, c_approx_cell - 1), min(GRID_SIZE, c_approx_cell + 2)):
                if 0 < r_wall_check < GRID_SIZE:
        ...
        for c_wall_check in range(max(0, c_approx_cell), min(GRID_SIZE + 1, c_approx_cell + 2)):
            for r_cell_check in range(max(0, r_approx_cell - 1), min(GRID_SIZE, r_approx_cell + 2)):
                if 0 < c_wall_check < GRID_SIZE:
    ...
    def draw_maze(self):
        ...
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                ...
                fill_color = COLOR_BACKGROUND; is_goal = (r, c) in self.goal_cells
        ...
        if start_r is not None and start_c is not None:
            ...
            arrow_tip_y = arrow_base_y - cell_size * 0.4; arrow_width = max(1, int(cell_size * 0.15))
    ...
    def find_and_draw_routes(self):
        """Calculates all route types, vertices, updates labels, and triggers drawing."""
        ...
        current_turn_weight = self.turn_weight_var.get() # Assumes validated by trace callback
        ...
        # Calculate Pixel Vertices for the Straightest Path
        ...
        # Update key labels
        w_text = f"(w={current_turn_weight:.1f})"
        ...
        self.key_label_smoothed.config(text=f"Sm: {self.msg_smoothed}")

    def _calculate_pixel_vertices(self, path):
        """Calculates the list of (x,y) pixel vertices for a given cell path."""
        if not path: return []
        vertices = []
    ...
    def _calculate_left_hand_path(self):
        ...
        while step_count < max_steps:
            ...
            if not (0 <= next_r < GRID_SIZE and 0 <= next_c < GRID_SIZE): next_dir = (direction + 1) % 4; next_r, next_c = r, c
        ...
        while pq:
            ...
            for next_dir8 in range(8):
                ...
                if move_ok:
                    ...
                    if next_visited_key not in visited or new_priority < visited[next_visited_key]:
                        new_path = path + [(next_r, next_c)]
                        heapq.heappush(pq, (new_priority, new_cost, new_turns, next_r, next_c, next_dir8, new_path))
        return [], "Unreachable"

    # --- Drawing Routes (Decoupled Version with Scaled Smoothing) ---
    def draw_current_routes(self):
        """Draws calculated routes based on visibility toggles and pre-calculated vertices."""
        self.canvas.delete("route_left", "route_shortest", "route_straightest",
                           "route_diagonal", "route_smoothed")
        visibility_map = {
            "route_left": self.show_route_left_var, "route_shortest": self.show_route_shortest_var,
            "route_diagonal": self.show_route_diagonal_var, "route_straightest": self.show_route_straightest_var,
            "route_smoothed": self.show_route_smoothed_var
        }
        ...
        line_options_straightest = {'width': ROUTE_WIDTH, 'capstyle': tk.ROUND}
        line_options_smoothed = {'width': ROUTE_WIDTH, 'capstyle': tk.ROUND, 'dash': (4, 4)}

        # Draw Basic Paths (Left, Shortest, Diagonal)
        ...
        # Draw Straightest Path (Orange Line) - Uses pre-calculated vertices
        if visibility_map["route_straightest"].get() and len(self.straightest_path_pixel_vertices) >= 2:
            ...
            self.canvas.create_line(straightest_points_flat, fill=COLOR_ROUTE_STRAIGHTEST,
                                    tags="route_straightest", **line_options_straightest)

        # Draw Smoothed Path (Cyan Dashed Line) - Also uses pre-calculated vertices
        if SCIPY_AVAILABLE and visibility_map["route_smoothed"].get() and len(self.straightest_path_pixel_vertices) >= 4:
            try:
                x_pts = [p[0] for p in self.straightest_path_pixel_vertices]
                y_pts = [p[1] for p in self.straightest_path_pixel_vertices]
                coords = np.array([x_pts, y_pts])

                # <<< Calculate Adjusted Smoothing Factor >>>
                current_cell_size = float(self.cell_visual_size_px)
                default_cell_size = float(DEFAULT_CELL_VISUAL_SIZE_PX)
                adjusted_smoothing_factor = BASE_SMOOTHING_FACTOR # Default
                if default_cell_size > 1e-6 and current_cell_size > 1e-6:
                    scale_factor_sq = (current_cell_size / default_cell_size) ** 2
                    adjusted_smoothing_factor = BASE_SMOOTHING_FACTOR * scale_factor_sq
                adjusted_smoothing_factor = max(0, adjusted_smoothing_factor)
                # <<< End Calculation >>>

                tck, u = splprep(coords, s=adjusted_smoothing_factor, k=3, per=0) # <<< Use adjusted factor
                num_smooth_points = max(50, len(self.straightest_path_pixel_vertices) * 5)
                u_fine = np.linspace(u.min(), u.max(), num_smooth_points)
                x_smooth, y_smooth = splev(u_fine, tck)
                smooth_points_flat = [coord for i in range(num_smooth_points) for coord in (x_smooth[i], y_smooth[i])]
                if len(smooth_points_flat) >= 4:
                    self.canvas.create_line(smooth_points_flat, fill=COLOR_ROUTE_SMOOTHED,
                                            tags="route_smoothed", **line_options_smoothed)
            except Exception as e:
                print(f"Error generating smoothed route: {e}")
                self.update_status("Error generating smoothed route.")
    ...
    def _recursive_backtracker_generate(self):
        ...
        if gen_start_r == GRID_SIZE - 1 and gen_start_c == 0:
            ...
            if 0 <= first_move_r < GRID_SIZE and 0 <= first_move_c < GRID_SIZE:
                 visited[first_move_r][first_move_c] = True; stack.append((first_move_r, first_move_c))
        while stack:
            r, c = stack[-1]; neighbours = self._get_neighbours(r, c, visited)
            if neighbours:
                (nr, nc), direction = random.choice(neighbours)
                self._remove_wall(r, c, direction); visited[nr][nc] = True; stack.append((nr, nc))
            else: stack.pop()
    ...
    def _is_true_center_post(self, r_post, c_post): return r_post == GRID_SIZE // 2 and c_post == GRID_SIZE // 2
    def add_wall_safe(self, wall_type, r_idx, c_idx):
        try:
            if wall_type == 'h':
                ...
                if 0 < r_idx < GRID_SIZE and 0 <= c_idx < GRID_SIZE: self.h_walls[r_idx][c_idx] = True
            elif wall_type == 'v':
                if self._is_true_center_post(r_idx, c_idx) or self._is_true_center_post(r_idx + 1, c_idx): return
                if 0 <= r_idx < GRID_SIZE and 0 < c_idx < GRID_SIZE: self.v_walls[r_idx][c_idx] = True
        except IndexError: pass
    def remove_walls_around_post(self, r_post, c_post):
        try:
            if c_post > 0: self.h_walls[r_post][c_post-1] = False
            if c_post < GRID_SIZE: self.h_walls[r_post][c_post] = False
            if r_post > 0: self.v_walls[r_post-1][c_post] = False
            if r_post < GRID_SIZE: self.v_walls[r_post][c_post] = False
        except IndexError: pass
    def _ensure_post_connectivity(self):
        center_r, center_c = GRID_SIZE // 2, GRID_SIZE // 2
        for r_post in range(1, GRID_SIZE):
            for c_post in range(1, GRID_SIZE):
                if r_post == center_r and c_post == center_c: continue
                connected = 0; possible = []
                # Use walrus operator carefully for assignment within condition
                if c_post > 0: (connected := connected + 1) if self.h_walls[r_post][c_post-1] else possible.append(('h', r_post, c_post-1))
                if c_post < GRID_SIZE: (connected := connected + 1) if self.h_walls[r_post][c_post] else possible.append(('h', r_post, c_post))
                if r_post > 0: (connected := connected + 1) if self.v_walls[r_post-1][c_post] else possible.append(('v', r_post-1, c_post))
                if r_post < GRID_SIZE: (connected := connected + 1) if self.v_walls[r_post][c_post] else possible.append(('v', r_post, c_post))
                if connected == 0 and possible: self.add_wall_safe(*random.choice(possible)) # Use * to unpack tuple args
    ...
    # <<< Corrected _ensure_single_goal_entry >>>
    def _ensure_single_goal_entry(self):
        ...
        perimeter_walls_defs = [('h', R_start, C_start), ('h', R_start, C_start+1), ('h', R_start+2, C_start), ('h', R_start+2, C_start+1), ('v', R_start, C_start), ('v', R_start+1, C_start),('v', R_start, C_start+2), ('v', R_start+1, C_start+2)]
        ...
        for gr, gc in local_goal_cells:
            for dir4 in range(4):
                 ...
                 if (0 <= adj_r < GRID_SIZE and 0 <= adj_c < GRID_SIZE and (adj_r, adj_c) not in local_goal_cells):
                     ...
                     # --- End Correction ---
                     if wall_info and wall_info not in candidate_walls: candidate_walls[wall_info] = (adj_r, adj_c)
        reachable_walls_to_open = [w for w, (ar, ac) in candidate_walls.items() if self._is_reachable(ar, ac)]
        wall_to_remove = None
        if reachable_walls_to_open: wall_to_remove = random.choice(reachable_walls_to_open)
        elif candidate_walls: print("WARNING: No reachable cells adjacent to goal! Forcing entry."); wall_to_remove = random.choice(list(candidate_walls.keys()))
        else: print("ERROR: No candidate walls found for goal entry!")
        if wall_to_remove:
            wt, wr, wc = wall_to_remove
            try:
                if wt == 'h': self.h_walls[wr][wc] = False
                elif wt == 'v': self.v_walls[wr][wc] = False
            except IndexError: pass
        self.remove_walls_around_post(center_r, center_c)

    def generate_maze(self):
        if not self._check_save_before_action("generating"): return
        for attempt in range(1, 21):
            self.update_status(f"Generating maze (Attempt {attempt})..."); self.master.update()
            self._initialize_all_walls(); self._recursive_backtracker_generate()
            self._add_loops(MAZE_GENERATION_LOOP_PROBABILITY); self._ensure_post_connectivity(); self._ensure_single_goal_entry()
            start_r, start_c = self.start_cell
            try: self.h_walls[start_r][start_c]=False; self.v_walls[start_r][start_c+1]=True
            except IndexError: pass
            path_data = self._calculate_dijkstra_path(0.0)
            goal_reachable = path_data[0] and any(g == path_data[0][-1] for g in self.goal_cells)
            if goal_reachable:
        ...
        messagebox.showerror("Generation Error", f"Failed after 20 attempts.", parent=self.master); self.update_status("Generation failed.");

    # --- Save/Load Text Format ---
    def save_maze_text(self):
        filename = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text files", "*.txt"), ("Maze files", "*.maze"), ("All files", "*.*")], title="Save Maze As Text File", initialfile="maze_" + time.strftime("%Y%m%d_%H%M%S") + ".txt")
        if not filename: return False
        output_lines = []
        for out_r in range(2 * GRID_SIZE + 1):
            row_str = ""; r_wall = out_r // 2; r_cell = (out_r - 1) // 2
            if out_r % 2 == 0:
                row_str += "o"
                for c in range(GRID_SIZE):
                    has_wall = False
                    if 0 <= r_wall < len(self.h_walls) and 0 <= c < len(self.h_walls[0]): has_wall = self.h_walls[r_wall][c]
                    row_str += ("---" if has_wall else "   ") + "o"
            else:
                for c in range(GRID_SIZE + 1):
                    has_wall = False
                    if 0 <= r_cell < len(self.v_walls) and 0 <= c < len(self.v_walls[0]): has_wall = self.v_walls[r_cell][c]
                    row_str += "|" if has_wall else " "
                    if c < GRID_SIZE: row_str += (" S " if (r_cell,c)==self.start_cell else (" G " if (r_cell,c) in self.goal_cells else "   "))
    ...
    def load_maze_text(self):
        ...
        try:
            ...
            rows, cols = 2*GRID_SIZE+1, 4*GRID_SIZE+1
            if len(lines) != rows: raise ValueError(f"Invalid rows. Expected {rows}, found {len(lines)}.")
            if not lines or len(lines[0]) < cols: raise ValueError(f"Invalid cols. Expected >= {cols}, found {len(lines[0]) if lines else 0}.")
            new_h=[[False for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE + 1)]; new_v=[[False for _ in range(GRID_SIZE + 1)] for _ in range(GRID_SIZE)]
            for r_idx, line in enumerate(lines):
                if r_idx%2==0: # H walls
                    r_wall = r_idx//2
                    if 0<=r_wall<=GRID_SIZE:
                        for c in range(GRID_SIZE): char_idx = c * 4 + 2; new_h[r_wall][c] = (line[char_idx] == '-') if char_idx < len(line) else False
                else: # V walls
                    r_cell=(r_idx-1)//2
                    if 0<=r_cell<GRID_SIZE:
                        for c in range(GRID_SIZE+1): char_idx = c * 4; new_v[r_cell][c] = (line[char_idx] == '|') if char_idx < len(line) else False
            self.h_walls=new_h; self.v_walls=new_v; self.initialize_outer_walls()
            self.current_maze_file=filename; self.maze_modified=False; self._update_window_title(); self.find_and_draw_routes()
            self.update_status(f"Loaded maze: {os.path.basename(filename)}")
    ...
    def fetch_github_maze_list(self):
        ...
        url = "https://api.github.com/repos/micromouseonline/mazefiles/contents/classic"; self.update_status("Fetching list..."); self.master.update()
        try:
            ...
            files={i['name']:i['download_url'] for i in data if isinstance(i,dict) and i.get('type')=='file' and i.get('name','').lower().endswith((".txt",".maze")) and i.get('download_url')}
            if not files: messagebox.showinfo("Not Found", "No .txt/.maze files found.", parent=self.master); self.update_status("No mazes found."); return
            self.update_status("List fetched. Select maze."); self._show_download_selection_dialog(files)
        except requests.exceptions.Timeout: messagebox.showerror("Fetch Error", "Timeout.", parent=self.master); self.update_status("Fetch failed (timeout).")
        except requests.exceptions.RequestException as e: messagebox.showerror("Fetch Error", f"Network error:\n{e}", parent=self.master); self.update_status("Fetch failed (network).")
        except ValueError as e: messagebox.showerror("Fetch Error", f"API error:\n{e}", parent=self.master); self.update_status("Fetch failed (API).")
        except Exception as e: messagebox.showerror("Fetch Error", f"Error:\n{e}", parent=self.master); self.update_status("Fetch failed.")

    # <<< Method with Preview Canvas and Logic >>>
    def _show_download_selection_dialog(self, maze_files):
        """Creates a Toplevel window to select a maze file, including a live preview."""
        dialog = Toplevel(self.master); dialog.title("Select Maze to Download"); dialog.geometry("550x450"); dialog.transient(self.master); dialog.grab_set()
        self.preview_canvas = None; self.preview_after_id = None; self.selected_maze_url = None # Reset state vars
        main_frame = Frame(dialog); main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        main_frame.rowconfigure(1, weight=1); main_frame.columnconfigure(0, weight=1); main_frame.columnconfigure(1, weight=0)
        search_frame = Frame(main_frame); search_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=(0, 5))
        ...
        list_frame = Frame(main_frame); list_frame.grid(row=1, column=0, sticky="nsew", padx=(0, 5)); scrollbar = Scrollbar(list_frame, orient=tk.VERTICAL); scrollbar.pack(side=tk.RIGHT, fill=tk.Y); listbox = Listbox(list_frame, yscrollcommand=scrollbar.set, exportselection=False, selectmode=SINGLE); listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True); scrollbar.config(command=listbox.yview)
        preview_frame = Frame(main_frame, bd=1, relief=tk.SUNKEN); preview_frame.grid(row=1, column=1, sticky="nsew", padx=(5, 0)); PREVIEW_SIZE = 180; self.preview_canvas = Canvas(preview_frame, width=PREVIEW_SIZE, height=PREVIEW_SIZE, bg="white", bd=0, highlightthickness=0); self.preview_canvas.pack(padx=2, pady=2); self.preview_canvas.create_text(PREVIEW_SIZE/2, PREVIEW_SIZE/2, text="Select a maze\nto preview", justify=tk.CENTER, fill="grey")
        button_frame = Frame(main_frame); button_frame.grid(row=2, column=0, columnspan=2, pady=(10, 0)); Button(button_frame, text="Load Selected", command=lambda: on_load_confirm(listbox, maze_files, dialog), width=12).pack(side=tk.LEFT, padx=10); Button(button_frame, text="Cancel", command=dialog.destroy, width=12).pack(side=tk.LEFT, padx=10)
        sorted_filenames = sorted(maze_files.keys(), key=str.lower)
        def _update_listbox(*args):
            term=search_var.get().lower().strip(); listbox.delete(0,END); [listbox.insert(END,n) for n in sorted_filenames if not term or term in n.lower()]
        ...
        search_var.trace_add("write", _update_listbox); _update_listbox()
        def _fetch_and_draw_preview(filename, url):
            ...
            try:
                ...
                rows, cols = 2*GRID_SIZE+1, 4*GRID_SIZE+1
                if len(lines) != rows: raise ValueError(f"Invalid rows")
                if not lines or len(lines[0]) < cols: raise ValueError(f"Invalid cols")
                # --- CORRECTED PARSING ---
                h_w=[[False for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE+1)]
                v_w=[[False for _ in range(GRID_SIZE+1)] for _ in range(GRID_SIZE)]
                for r_idx, line in enumerate(lines):
                    if r_idx%2==0:
                        r_wall = r_idx//2
                        if 0<=r_wall<=GRID_SIZE:
                            for c in range(GRID_SIZE): char_idx=c*4+2; h_w[r_wall][c]=(line[char_idx]=='-') if char_idx<len(line) else False
                    else:
                        r_cell=(r_idx-1)//2
                        if 0<=r_cell<GRID_SIZE:
                            for c in range(GRID_SIZE+1): char_idx=c*4; v_w[r_cell][c]=(line[char_idx]=='|') if char_idx<len(line) else False
                # --- END CORRECTION ---
                self._draw_maze_on_canvas(self.preview_canvas, h_w, v_w, PREVIEW_SIZE)
        ...
        def _on_preview_selection(event=None):
            ...
            filename = listbox.get(idx[0]); url = maze_files.get(filename); self.selected_maze_url = url
        ...
        def on_load_confirm(lb, files_dict, dlg):
            idx = lb.curselection()
            if not idx: messagebox.showwarning("No Selection", "Please select a maze file.", parent=dlg); return
            filename = lb.get(idx[0]); url = files_dict.get(filename)
            if url: dlg.destroy(); self._download_and_load_selected_maze(filename, url)
            else: messagebox.showerror("Internal Error", f"URL not found for '{filename}'.", parent=dlg)
        listbox.bind("<Double-1>", lambda e:on_load_confirm(listbox, maze_files, dialog)); listbox.bind("<Return>", lambda e:on_load_confirm(listbox, maze_files, dialog)); search_entry.focus_set(); dialog.wait_window()

    # <<< New Helper Function >>>
    def _draw_maze_on_canvas(self, target_canvas, h_walls, v_walls, target_size_px):
        """Draws a simplified maze representation onto a given canvas."""
        target_canvas.delete("all")
        if not h_walls or not v_walls or target_size_px <= 10:
             target_canvas.create_text(target_size_px/2, target_size_px/2, text="Invalid Data\nfor Preview", justify=tk.CENTER, fill="orange")
             return
        preview_margin = max(5, int(target_size_px * 0.05)); drawable_size = target_size_px - 2 * preview_margin
        if drawable_size <= 0: return
        cell_size = drawable_size / GRID_SIZE
    ...
    # <<< Corrected _download_and_load_selected_maze >>>
    def _download_and_load_selected_maze(self, filename, download_url):
        """Downloads the content of the selected maze file and loads it."""
        ...
        try:
            ...
            rows, cols = 2*GRID_SIZE+1, 4*GRID_SIZE+1
            if len(lines) != rows: raise ValueError(f"Invalid rows: {len(lines)} vs {rows}")
            if not lines or len(lines[0]) < cols: raise ValueError(f"Invalid cols: {len(lines[0]) if lines else 0} vs {cols}")
            new_h=[[False for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE + 1)]; new_v=[[False for _ in range(GRID_SIZE + 1)] for _ in range(GRID_SIZE)]
            for r_idx, line in enumerate(lines):
                if r_idx%2==0: # Even line -> Horizontal walls
                    r_wall = r_idx//2
                    if 0<=r_wall<=GRID_SIZE:
                        for c in range(GRID_SIZE): char_idx = c * 4 + 2; new_h[r_wall][c] = (line[char_idx] == '-') if char_idx < len(line) else False
                else: # Odd line -> Vertical walls
                    r_cell=(r_idx-1)//2
                    if 0<=r_cell<GRID_SIZE:
                        for c in range(GRID_SIZE+1): char_idx = c * 4; new_v[r_cell][c] = (line[char_idx] == '|') if char_idx < len(line) else False
            self.h_walls=new_h; self.v_walls=new_v; self.initialize_outer_walls()
            self.current_maze_file = f"GitHub: {filename}"; self.maze_modified=False; self._update_window_title(); self.find_and_draw_routes()
            self.update_status(f"Loaded '{filename}' from GitHub.")
        ...
        except ValueError as e: messagebox.showerror("Error", f"Invalid format '{filename}':\n{e}", parent=self.master); self.update_status(f"Load failed: {e}"); self.current_maze_file=None; self.maze_modified=False; self._update_window_title()
